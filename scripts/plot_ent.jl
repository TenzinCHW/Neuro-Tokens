...
function save_plot_hm(hm, loc, min_val, max_val)
    ...
    hm = hm[:, :, 1]
    Plots.heatmap(10:5:50, 500:500:5000, hm, xlabel="window size / number of timesteps",  ylabel="bin size / μs", clim=(min_val, max_val))
    Plots.savefig(loc)
end
...
loc = DrWatson.plotsdir("raw_entropy.$ext")
save_plot_hm(raw_ent, loc, min_val, max_val)
loc = DrWatson.plotsdir("model_entropy.$ext")
